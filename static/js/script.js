$(document).ready(function() {
    ...
    $("#productId").on("change", function(){
        ...
        if ($("#productId").val()) {
            ajaxCall("getLocations/");
            enableOptions();
            $("#fromLocationSubs").prop("disabled", false);
        } else {
          disableOptions();
        }
        return false;
    });
    $("#productIdSubs").on("change", function(){
      $("#fromLocationSubs option").not(":first").remove();
      if ($("#productIdSubs").val()) {
          ajaxCallSubs("getLocations/substract/");
          $("#qtySubs").prop("disabled", false);
          $("#fromLocationSubs").prop("disabled", false);
    ...
    function disableOptions()
    {
        ...
        $("#qtySubs").prop("disabled", "disabled");
        $("#fromLocationSubs").prop("disabled", "disabled");
    ...
    function ajaxCallSubs(table){
      $.ajax({
        data: {
          productIdSubs: $("#productIdSubs").val(),
          locationSubs: $("#fromLocationSubs").val(),
        },
        type: "POST",
        url: table,
      }).done(function (data) {
        $.each(data, function (index,value){
            $("#fromLocationSubs").append(
              $("<option>", {
                value: index,
                text: index,
                "data-max": value.qty,
