...
@app.route("/movements/substract/", methods=["POST"])
def substractFromMovements():
    if request.method == "POST" :
        product_id      = request.form["productIdSubs"]
        qty             = -int(request.form["qtySubs"])
        fromLocation    = request.form["fromLocationSubs"]
        new_movement = ProductMovement(
            product_id=product_id, qty=qty, from_location=fromLocation, to_location="NA")
        ...
        try:
            ...
            return "There Was an issue while substracting"
...
@app.route("/movements/getLocations/", methods=["POST"])
def getLocations():
...
@app.route("/movements/getLocations/substract/", methods=["POST"])
def getLocationsSubs():
    product = request.form["productIdSubs"]
    location = request.form["locationSubs"]
    locationDict = defaultdict(lambda: defaultdict(dict))
    locations = ProductMovement.query.\
        filter( ProductMovement.product_id == product).\
        filter(ProductMovement.to_location != '').\
